...
| `SUPABASE_*`          | Interpretation fine; nothing persisted; `/api/commands` and `/api/memory` `503`. |
...
Vercel project with **Root Directory = `backend`**. Node 24.x (`engines`). `vercel.json` pins
